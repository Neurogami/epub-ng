...
module Neurogami
  class EpubNg
    ...
    # see: http://www.mimemap.com/mime/lis
    MEDIATYPES = {}

    {
        %w{html xhtml xml } => 'application/xhtml+xml',
      %w{css } => 'text/css',
      %w{ png } => 'image/png',
      %w{ jpg } => 'image/jpg',
      %w{ ncx } => 'application/x-dtbncx+xml',
      %w{xpgt } => 'application/vnd.adobe-page-template+xml',
      %w{otf } => 'application/x-font-opentype',
      %w{svg} => 'image/svg+xml	svg',
      %w{ttf} => 'application/x-font-ttf',
      %w{woff} => 'application/x-font-woff',
      %w{eot } => 'application/vnd.ms-fontobject'
    }.each { |k, v| 
          k.each {|ext| 
            MEDIATYPES[ext] = v 
          }
    ...
    <metadata xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:opf="http://www.idpf.org/2007/opf">
     ...
     ~
        ...
        def media_type file
          # Also, see: http://www.mimemap.com/mime/list
          ext = file.split('.').last
          _ = MEDIATYPES[ext]
          raise "Unknown file type in media_type for file '#{file}'" unless _
          _
        end
